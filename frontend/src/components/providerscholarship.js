...
import { getProviderScholarships } from "../services/ScholarshipService";
...
function ProviderScholarship({ routes }) {
  ...
  useEffect(() => {
    getProviderScholarships(cookies.p_id).then((data) => setScholarships(data));
  }, []);
  ...
  const actionBodyTemplate = (row) => {
    return (
      <React.Fragment>
          <Button label="Apply" onClick={apply} className="w-6" />
      </React.Fragment>
    );
  };
...
export default ProviderScholarship;
