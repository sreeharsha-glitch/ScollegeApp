...
function ProviderApplication({ routes }) {
  ...
  useEffect(() => {
    getApplications("p_id",cookies.p_id).then((data) => setApplications(data));
  }, []);

  const actionBodyTemplate = (row) => {
    const toLink = `/provider-application/${row?.app_id}`;
    return (
      <React.Fragment>
        <Link
        ...
        >
          <Button label="View/Modify" className="w-6" />
        </Link>
      </React.Fragment>
    );
  };
